...
const feedFileName = "world-cup-2026-calendar.zh.ics";
...
await fs.writeFile(path.join(root, feedFileName), ics, "utf8");
await fs.writeFile(path.join(root, "index.html"), indexHtml(feedFileName), "utf8");
await fs.writeFile(path.join(root, ".nojekyll"), "", "utf8");

console.log(
  offline
    ? `Wrote placeholder feed to ${feedFileName}`
    : `Wrote ${matchCount} matches to ${feedFileName}`
);
